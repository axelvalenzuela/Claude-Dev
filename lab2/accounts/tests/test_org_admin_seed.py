"""The migrations that seed the org-chart accounts — Iris Cortez (HR admin),
Adrian Heymes (ICS admin), Karen Plascencia (HR admin), Steffan Widmer
(HR admin, CEO), and Axel Valenzuela's MHP-domain identity (a regular
employee, not an admin) — always run for the test database, so we can
...
0008_seed_axel_mhp_employee.py and 0009_seed_karen_admin.py for why the
...
class OrgAdminSeedTests(TestCase):
    ...
    def test_axel_mhp_identity_is_a_regular_employee_not_an_admin(self):
        axel = User.objects.get(email="axel.valenzuela@mhp.com")
        self.assertFalse(axel.is_staff)
        self.assertFalse(axel.is_superuser)
    ...
    def test_steffan_is_a_superuser_with_no_department_scope(self):
        steffan = User.objects.get(email="steffan.widmer@mhp.com")
        self.assertTrue(steffan.is_staff)
        self.assertTrue(steffan.is_superuser)
        self.assertEqual(steffan.supervised_department, "")
        self.assertIsNotNone(steffan.employee_number)

    def test_every_seeded_org_account_has_a_distinct_employee_number(self):
        numbers = list(
            User.objects.filter(
                email__in=[
                    "iris.cortez@mhp.com",
