...
Adrian Heymes (ICS admin), Karen Plascencia (HR admin), and Axel
Valenzuela's MHP-domain identity (a regular employee, not an admin) —
always run for the test database, so we can assert on their result
directly. None of these assert on password values: .env is loaded by
settings.py before migrations run, so the test database is actually
seeded with whatever real password is in .env, not each migration's
committed placeholder default — see accounts/migrations/
